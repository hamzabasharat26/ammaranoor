...
export default function LightSections() {
  ...
  return (
    <Reveal selector=".lt-reveal">
      <div id="light-band" data-theme="light" className="relative z-10 px-3 py-10 md:px-6 md:py-16">
        {/* ---------- About ---------- */}
        <section
          id="about"
            ...
            <div className="lt-reveal mt-12 grid grid-cols-1 gap-8 border-t border-line pt-10 sm:grid-cols-2">
              ...
              <div>
                ...
                <ul className="mt-4 space-y-2 text-sm leading-relaxed text-fg-dim">
                  {leadership.map((l) => (
                    <li key={`${l.role}-${l.org}`}>
                      <span className="font-semibold text-fg">{l.role}</span> —{" "}
                      {l.org}
                      {l.period ? ` (${l.period})` : ""}
                    </li>
                  ))}
